defmodule SnickrWeb.ChannelController do
  ...
  def new(conn, %{"workspace_id" => workspace_id}) do
    ...
    # workspace
    render(conn, "new.html", workspace_id: workspace_id,
      changeset: Channel.create_changeset(%Channel{}, conn.assigns.current_user.id, workspace_id, %{}))
  end
  ...
  def show(conn, %{"id" => id}) do
    case Platform.get_channel_with_subscriber(conn.assigns.current_user, id) do
      ...
      channel ->
        render(conn, "show.html", channel: channel, changeset: Message.changeset(%Message{}, %{channel_id: channel.id}))
    end
  end
end
